...
export function RegistrationForm() {
  ...
  const navigate = useNavigate();
  const passwordValid = password.length >= 8 && /[a-z]/.test(password) && /[A-Z]/.test(password) && /\d/.test(password) && /[^A-Za-z0-9]/.test(password);
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); setSubmitted(true);
    const form = new FormData(event.currentTarget);
    if (!form.get("universityId") || !form.get("email") || !form.get("firstName") || !form.get("lastName") || !form.get("dateOfBirth") || !form.get("gender") || !form.get("jambNumber") || !form.get("department") || !form.get("faculty") || !form.get("confirm") || !passwordValid || password !== confirmPassword) return;
    setLoading(true); window.setTimeout(() => void navigate({ to: "/verify", search: { type } }), 800);
  };
  return <form onSubmit={submit} className="space-y-5" noValidate><UserTypeTabs value={type} onChange={(value) => { setType(value); setSubmitted(false); }} /><div className="grid gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Field id="universityId" name="universityId" label={type === "student" ? "Matriculation Number" : "Staff ID"} placeholder={type === "student" ? "Enter your matriculation number" : "Enter your staff ID"} error={submitted ? "Enter your university ID." : undefined} /></div><div className="sm:col-span-2"><Field id="email" name="email" type="email" label={type === "student" ? "Personal Email Address" : "Institutional Email Address"} placeholder={type === "student" ? "Enter your personal email address" : "Enter your institutional email address"} error={submitted ? (type === "student" ? "Enter your personal email address." : "Enter your institutional email address.") : undefined} /></div><Field id="firstName" name="firstName" label="First Name" placeholder="First name" error={submitted ? "Enter your first name." : undefined} /><Field id="lastName" name="lastName" label="Last Name" placeholder="Last name" error={submitted ? "Enter your last name." : undefined} /><Field id="dateOfBirth" name="dateOfBirth" type="date" label="Date of Birth" placeholder="" error={submitted ? "Enter your date of birth." : undefined} /><PortalSelect name="gender" label="Gender" placeholder="Select gender" values={["Female", "Male", "Prefer not to say"]} /><PortalSelect name="department" label="Department" placeholder="Select department" values={departments} /><PortalSelect name="faculty" label={type === "student" ? "Faculty" : "Faculty / Unit"} placeholder="Select faculty or unit" values={faculties} /><Field id="phone" name="phone" label="Phone Number" placeholder="e.g. 0801 234 5678" optional /><Field id="jambNumber" name="jambNumber" label={type === "student" ? "JAMB Registration Number" : "NIN"} placeholder={type === "student" ? "Enter your JAMB registration number" : "Enter your National Identification Number (NIN)"} error={submitted ? (type === "student" ? "Enter your JAMB registration number." : "Enter your NIN.") : undefined} /><div className="sm:col-span-2 space-y-5"><PasswordField id="password" label="Create Password" value={password} onChange={setPassword} placeholder="Create a password" error={submitted && !passwordValid ? "Password does not meet all requirements." : undefined} /><PasswordRequirements password={password} /><PasswordField id="confirmPassword" label="Confirm Password" value={confirmPassword} onChange={setConfirmPassword} placeholder="Enter password again" error={submitted && password !== confirmPassword ? "Passwords do not match." : undefined} /></div></div><label className="flex cursor-pointer items-start gap-3 text-sm leading-5"><Checkbox name="confirm" value="yes" className="mt-0.5" /><span>I confirm that the information provided belongs to me.</span></label>{submitted && <FormAlert title="Check your information">Please complete the required fields and confirm the information belongs to you.</FormAlert>}<Button className="h-11 w-full" disabled={loading}>{loading ? <><Loader2 className="animate-spin" />Registering...</> : "Continue"}</Button></form>;
}
